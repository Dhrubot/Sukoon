...
export const COUNTRY_OPTIONS: CountryOption[] = [
  ...
  { code: 'GR', name: 'Greece' },
  { code: 'GS', name: 'So. Georgia & So. Sandwich Isl.' },
  ...
  { code: 'IN', name: 'India' },
  { code: 'IO', name: 'Chagos Archipelago' },
  { code: 'IQ', name: 'Iraq' },
  { code: 'IR', name: 'Iran' },
...
export function filterCountryOptions(query: string, limit = 8): CountryOption[] {
  ...
  return COUNTRY_OPTIONS
    .map((country) => {
      ...
      if (normalizedName === normalizedQuery || country.code.toLowerCase() === normalizedQuery) {
        score = 100;
...
export function findCountryOptionByCode(code?: string | null): CountryOption | null {
  if (!code) return null;
  const normalizedCode = code.trim().toUpperCase();
  return COUNTRY_OPTIONS.find((country) => country.code === normalizedCode) || null;
}

export function findCountryOptionByName(name?: string | null): CountryOption | null {
  if (!name) return null;
  const normalizedName = normalizeCountryText(name);
  return COUNTRY_OPTIONS.find((country) => normalizeCountryText(country.name) === normalizedName) || null;
}
