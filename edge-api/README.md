...
2. Run `npm run build:city-index` after changing `data/cities.v2.json`.
   Use `npm run build:city-index:v1` only when you need to rebuild the legacy shard set.
...
- City search now reads KV-backed shards generated from a versioned source file such as `data/cities.v2.json`.
...
- `CITY_INDEX_VERSION` lets you roll forward to a new shard set without breaking old keys. The worker default is now `v2`.
- The build script enforces shard guardrails and prints the largest shard size on every run.
...
- If you pipe `wrangler tail` output into a file, the review script will group misses by `country + queryPrefix` so you can expand the active `data/cities.<version>.json` source based on real demand.
